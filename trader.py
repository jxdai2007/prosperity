...
MM_DYNAMIC_EMA_ALPHA = 0.5   # EMA smoothing for dynamic fair value (more reactive)
    ...
    def options_trade(self, product: str, state: TradingState, saved: dict) -> list:
        ...
        if underlying == "COCONUT":
            ...
            if sell_qty > 0 and sell_price > 0:
                orders.append(Order(product, sell_price, -sell_qty))
        # P3 options: no MM resting orders, only take mispriced (avoid adverse selection)
